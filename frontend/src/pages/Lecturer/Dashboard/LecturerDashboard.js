...
function LecturerDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const userResponse = await httpClient.get("https://gradex-6c6911643a2a.herokuapp.com/@me");
        setUser(userResponse.data);

        const performanceResponse = await httpClient.get(`https://gradex-6c6911643a2a.herokuapp.com/get_topics_performance/1`);
        console.log("Performance Response:", performanceResponse.data);
        setPerformanceData(performanceResponse.data);

        // Fetch student names and ids for suggestions
        const studentsResponse = await httpClient.get("https://gradex-6c6911643a2a.herokuapp.com/get_student_names_ids");
  ...
  useEffect(() => {
    if (selectedStudentId) {
      (async () => {
        try {
          const studentPerformanceResponse = await httpClient.get(`https://gradex-6c6911643a2a.herokuapp.com/get_performance/${selectedStudentId}`);
